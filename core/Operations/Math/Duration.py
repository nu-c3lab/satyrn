...
class Duration(ArithmeticOperation, SQLAInterface, PandasInterface):
    def __init__(self):
        name = 'duration'
        input_args = [
            OperationArgument(1, 1, [ArgType.StartDate]),
            OperationArgument(1, 1, [ArgType.EndDate]),
        ]
        output_args = [
            OperationArgument(1, 1, [ArgType.Datetime])
        ]
        template = "length of time between {0} and {1}"
        super().__init__(name, input_args, output_args, template)

    def sqlalchemy_op(self,
                      operation_input: List[Any],
                      db_type: str) -> None:
        return reduce(lambda a, arg: arg - a, operation_input)
